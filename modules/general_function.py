...
def collect_position(filelist):
    newlist = [position(pos[0]) for pos in filelist]
...
def count_average(filelist):
    avg = round(statistics.mean(filelist), 2)
    filelist.append(avg)
    return filelist

# Counting total order
def count_total_order(filelist):
    totalOrder = len([fn['Total Item'].to_list() for fn in filelist])
    return count_average(totalOrder)

# Counting total item picked
def count_total_item(filelist):
    totalItemPicked = sum([sum(fn['Total Item'].to_list()) for fn in filelist])
    return count_average(totalItemPicked)
